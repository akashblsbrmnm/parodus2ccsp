                                ...
                                rbusMethodAsyncHandle_t asyncHandle)
{
    ...
    pCount = rbusProperty_Count(prop);

    WalInfo("SubscribeToNotifyParamHandler invoked\n");
    WalInfo("Total params to subscribe: %d", pCount);
    ...
    rbusEventSubscription_t* subscription = calloc(pCount, sizeof(rbusEventSubscription_t));
    if(!subscription)
    {
        WalError("Memory allocation failed for subscription\n");
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!val || rbusValue_GetType(val) != RBUS_STRING) continue;
        
        const char* strVal = rbusValue_GetString(val, NULL);
        if (!strVal || !*strVal) continue;
        
        subscription[count].eventName = strdup(strVal);
        subscription[count].filter = NULL;
        subscription[count].interval = 0;
        subscription[count].duration = 0;
        subscription[count].userData  = strdup(strVal);
        subscription[count].handler   = eventReceiveHandler;
        subscription[count].publishOnSubscribe = true;
        count++;
    }
    if (count == 0)
    {
    ...
    rbusError_t err = rbusEvent_SubscribeExAsync(handle, subscription, count, subscribeAsyncHandler, 0);
    ...
    rbusEventSubscription_t* subscription)
{
    ...
    { 
        ...
        if(oldValue)
        {
            ...
            WalInfo("New Value: %s Old Value: %s\n", newValueStr ? newValueStr : "NULL", oldValueStr ? oldValueStr : "NULL");
        }
    }
    else
    {
